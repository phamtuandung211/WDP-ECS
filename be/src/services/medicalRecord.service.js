...
export const createMedicalRecord = async ({
    ...
    if (appointment.status !== APPOINTMENT_STATUS.CONFIRMED) {
        throwErr(400, `Cannot create medical record for appointment with status: ${appointment.status}`);
    }
...
export const getMyMedicalRecords = async ({ accountId, page = 1, limit = 10 }) => {
    ...
    return {
        data: records,
        metadata: getPaginationMetadata(records.length, totalItems, safeLimit, offset),
    };
};
...
export const getMedicalRecordById = async ({ recordId, accountId, role }) => {
    ...
    if (role === "CUSTOMER") {
        const customer = await Customer.findOne({ accountId }).lean();
        if (!customer || record.customerId._id.toString() !== customer._id.toString()) {
...
export const updateMedicalRecord = async ({ recordId, accountId, payload }) => {
    ...
    const ALLOWED_FIELDS = ["symptoms", "diagnosis", "prescription", "notes", "aiSummary"];
    const updateData = {};
