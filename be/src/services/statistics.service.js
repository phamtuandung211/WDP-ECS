  ...
  const pipeline = [
...

    {
      $group: {
        ...
        confirmed: {
          $sum: {
            $cond: [{ $eq: ["$status", APPOINTMENT_STATUS.CONFIRMED] }, 1, 0],
          },
        },
        completed: {
          $sum: {
        ...
        canceled: {
          $sum: {
            $cond: [{ $eq: ["$status", APPOINTMENT_STATUS.CANCELED] }, 1, 0],
          },
        },
      },
    },
...
export const getCustomerStats = async ({ from, to, page = 1, limit = 10 }) => {
  const dateRange = buildDateRange(from, to);
  const matchStage = dateRange ? { createdAt: dateRange } : {};
  ...
  const pipeline = [
    { $match: { ...matchStage, customerId: { $ne: null } } },

    {
      $group: {
        ...
        pendingPayment: {
          $sum: {
            $cond: [
              { $eq: ["$status", APPOINTMENT_STATUS.PENDING_PAYMENT] },
              1,
              0,
            ],
          },
    ...
    { $sort: { totalAppointments: -1, completed: -1 } },
  ...
  const customerAgg = await Appointment.aggregate(pipeline);

  const totalCustomers = await Appointment.distinct("customerId", {
    ...matchStage,
    customerId: { $ne: null },
  });
...
export const getFeedbackStats = async ({ from, to }) => {
  ...
  return {
    avgRating: avgResult[0]?.avg
      ? parseFloat(avgResult[0].avg.toFixed(2))
      : null,
